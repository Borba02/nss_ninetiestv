...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            List<Show> orderedNames = shows.OrderByDescending(s => s.Name).ToList();
            return orderedNames;
        }
        ...
        {
            List<Show> orderedRating = shows.OrderByDescending(s => s.ImdbRating).ToList();
        ...
        {
            List<Show> topThreeRating = shows.OrderByDescending(s => s.ImdbRating).Take(3).ToList();
        ...
        {
            List<Show> excludeWorstShow = shows.OrderBy(s => s.ImdbRating).Skip(1).ToList();
            return excludeWorstShow;
        }
        ...
        {
            return shows.OrderBy(s => s.StartYear - s.EndYear).ToList();
        }
        ...
        {
            return shows.Where(s => s.EndYear - s.StartYear > 10).OrderBy(s => s.Name).First(s => s.ImdbRating < 8);
        }
